...
DFA DFA::fromNFA(const NFA &nfa, int &dfaStateCounter) {
    ...
    for (const auto &s : startSet) {
        if (s->isAccepting) {
            if (winningPriority == -1 ||
                (s->priority != -1 && s->priority < winningPriority)) {
    ...
    while (!worklist.empty()) {
        ...
        for (int c : inputs) {
            ...
            std::set<std::shared_ptr<State>> closureSet =
                _epsilonClosure(moveSet);
...
DFA::_epsilonClosure(const std::set<std::shared_ptr<State>> &states) {
...
std::set<std::shared_ptr<State>>
DFA::_move(const std::set<std::shared_ptr<State>> &states, int c) {
